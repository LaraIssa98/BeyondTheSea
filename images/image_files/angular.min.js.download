...
(function(y) {
  ...
  function Sc(a, b) {
    ...
    'string' === typeof a && '$' === a.charAt(0) && '$' === a.charAt(1)
      ...
      : Za(b)
        ? (d = '$WINDOW')
        : b && y.document === b ? (d = '$DOCUMENT') : bb(b) && (d = '$SCOPE');
    return d;
  }
  function eb(a, b) {
      ...
      return {
        invoke: function(a, b, c, d) {
          ...
          if (Ba || 'function' !== typeof d) d = !1;
            ...
            var g = d.$$ngIsClass;
            La(g) ||
              (g = d.$$ngIsClass = /^(?:class\b|constructor\()/.test(
      ...
      function(a, b, c, e, p, V, O, qa, L, r) {
        ...
        ba.$$addScopeInfo = n
          ? function(a, b, c, d) {
              a.data(
                c
                  ? d ? '$isolateScopeNoTemplate' : '$isolateScope'
                  : '$scope',
                b,
              );
            }
        ...
        c.onUrlChange(function(a, b) {
          zc(a, B)
            ? (d.$evalAsync(function() {
                ...
                l.$$parse(a);
                l.$$state = b;
                g = d.$broadcast('$locationChangeStart', a, c, b, e)
                  .defaultPrevented;
                l.absUrl() === a &&
                  (g
          ...
          if (q || l.$$urlUpdatedByLocation) {
            ...
            if (q || m)
              (q = !1),
                d.$evalAsync(function() {
                  var b = l.absUrl(),
                    c = d.$broadcast('$locationChangeStart', b, a, l.$$state, g)
                      .defaultPrevented;
                  l.absUrl() === b &&
                    (c
                      ? (l.$$parse(a), (l.$$state = g))
  ...
  var Ug = {
    ...
    Tg = /((?:[^yMLdHhmsaZEwG']+)|(?:'(?:[^']|'')*')|(?:E+|y+|M+|L+|d+|H+|h+|m+|s+|a|Z|G+|w+))([\s\S]*)/,
    Sg = /^-?\d+$/;
  Td.$inject = ['$locale'];
  var Ng = ka(N),
    Og = ka(ub);
  ...
  var ke = function(a) {
    ...
    Ke = ke(),
    We = ke(!0),
    Vg = /^\d{4,}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+(?:[+-][0-2]\d:[0-5]\d|Z)$/,
    gh = /^[a-z][a-z\d.+-]*:\/*(?:[^:@]+(?::[^@]+)?@)?(?:[^\s:/?#]+|\[[a-f\d:]+])(?::\d+)?(?:\/[^?#]*)?(?:\?[^#]*)?(?:#.*)?$/i,
    hh = /^(?=.{1,254}$)(?=.{1,64}@)[-!#$%&'*+/0-9=?A-Z^_`a-z{|}~]+(\.[-!#$%&'*+/0-9=?A-Z^_`a-z{|}~]+)*@[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*$/,
    Wg = /^\s*(-|\+)?(\d+|(\d*(\.\d*)))([eE][+-]?\d+)?\s*$/,
    le = /^(\d{4,})-(\d{2})-(\d{2})$/,
  ...
  var pf = function() {
    ...
    lh = M('ngOptions'),
    mh = /^\s*([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+group\s+by\s+([\s\S]+?))?(?:\s+disable\s+when\s+([\s\S]+?))?\s+for\s+(?:([$\w][$\w]*)|(?:\(\s*([$\w][$\w]*)\s*,\s*([$\w][$\w]*)\s*\)))\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?$/,
    jf = [
      '$compile',
      '$document',
      '$parse',
